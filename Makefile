...
run: env-check ## Run the agent locally
	@echo "🤖 Starting data-science-team-agent..."
	@uv run python -m data_science_team_agent

.PHONY: run-dev
run-dev: ## Run the agent in development mode with auto-reload
	@echo "🤖 Starting data-science-team-agent in development mode..."
	@uv run watchfiles "python -m data_science_team_agent" data_science_team_agent/
...
docker-build: ## Build Docker image
	...
	@docker build -f Dockerfile.agent -t data_science_team_agent:latest .

.PHONY: docker-build-no-cache
docker-build-no-cache: ## Build Docker image without cache
	@echo "🐳 Building Docker image (no cache)..."
	@docker build --no-cache -f Dockerfile.agent -t data_science_team_agent:latest .

.PHONY: docker-run
docker-run: ## Run Docker container with .env file
	@echo "🐳 Running Docker container..."
	@docker run -d \
		--name data_science_team_agent \
		-p 3773:3773 \
		--env-file .env \
		--restart unless-stopped \
		data_science_team_agent:latest
...
docker-stop: ## Stop Docker container
	...
	@docker stop data_science_team_agent || true
	@echo "✅ Container stopped"

.PHONY: docker-rm
docker-rm: docker-stop ## Remove Docker container
	@echo "🗑️  Removing Docker container..."
	@docker rm data_science_team_agent || true
	@echo "✅ Container removed"

.PHONY: docker-logs
docker-logs: ## View Docker container logs
	@docker logs -f data_science_team_agent

.PHONY: docker-shell
docker-shell: ## Open shell in running Docker container
	@docker exec -it data_science_team_agent bash
...
docker-clean: docker-rm ## Clean up Docker images and containers
	...
	@docker rmi data_science_team_agent:latest || true
...
docker-push: ## Push Docker image to Docker Hub
	...
	@docker tag data_science_team_agent:latest para5/data_science_team_agent:latest
	@docker push para5/data_science_team_agent:latest
...
status: ## Show agent and Docker status
	...
	@docker ps -a | grep data_science_team_agent || echo "No containers found"
	@echo "\nDocker Images:"
	@docker images | grep data_science_team_agent || echo "No images found"
	@echo "\nAgent Health:"
